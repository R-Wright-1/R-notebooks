...
## Commands run, reads remaining and domain-level classifications {.tabset}
...
def remove_symbol(df):
  ...
  for i in ind:
    if '[' in i:
      ind_dict[i] = i.replace('[', '').replace(']', '')
  df = df.rename(index=ind_dict)
...
bracken, kraken, kraken_percent, bracken_fungi, bracken_fungi_ra = remove_symbol(bracken), remove_symbol(kraken), remove_symbol(kraken_percent), remove_symbol(bracken_fungi), remove_symbol(bracken_fungi_ra)
...
kraken_reads_W0 = pd.DataFrame(kraken_reads.sum(axis=0)).transpose()
plt.tight_layout()
plt.show()
```
...
plot_domain(kraken_reads, kraken_percent)
kraken_reads_W06 = pd.DataFrame(kraken_reads.sum(axis=0)).transpose()
plt.tight_layout()
plt.show()
```
...
plot_domain(kraken_reads, kraken_percent)
kraken_reads_W12 = pd.DataFrame(kraken_reads.sum(axis=0)).transpose()
plt.tight_layout()
plt.show()
```
...
Using [KrakenTools](https://github.com/jenniferlu717/KrakenTools.git) - this searches based on the NCBI fungal taxon ID ([4751](https://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?id=4751&lvl=0)) and then also takes all lower classifications.
```{bash, eval=FALSE}
...
<ul>
<li>W0_kneaddata_read_counts.txt
<li>W06_kneaddata_read_counts.txt
<li>W12_kneaddata_read_counts.txt
</ul>
...
<ul>
<li>W0_full_species.csv
<li>W06_full_species.csv
<li>W12_full_species.csv
</ul>

**Fungal taxonomy tables (number of reads):**
<ul>
<li>W0_bracken_fungi.csv
<li>W06_bracken_fungi.csv
<li>W12_bracken_fungi.csv
</ul>

**Fungal taxonomy tables (relative abundance):**
<ul>
<li>W0_bracken_fungi_relabun.csv
<li>W06_bracken_fungi_relabun.csv
<li>W12_bracken_fungi_relabun.csv
</ul>

**HUMAnN output:**
<ul>
<li>*Gene families (cpm)*
<ul>
<li>W0/humann_genefamilies_cpm.tsv
<li>W06/humann_genefamilies_cpm.tsv
<li>W12/humann_genefamilies_cpm.tsv
</ul>
<li>*KEGG orthologs (Uniref50 mapping)*
<ul>
<li>W0/humann_genefamilies_cpm_ko50.tsv
<li>W06/humann_genefamilies_cpm_ko50.tsv
<li>W12/humann_genefamilies_cpm_ko50.tsv
</ul>
<li>*KEGG orthologs (Uniref90 mapping)*
<ul>
<li>W0/humann_genefamilies_cpm_ko90.tsv
<li>W06/humann_genefamilies_cpm_ko90.tsv
<li>W12/humann_genefamilies_cpm_ko90.tsv
</ul>
<li>*Pathways*
<ul>
<li>W0/humann_pathabundance_relab.tsv
<li>W06/humann_pathabundance_relab.tsv
<li>W12/humann_pathabundance_relab.tsv
</ul></ul>
...
### Stacked bar fungi (genus, relative abundance)

Here all fungi have been grouped to the genus level and this shows all genera that are above 2% abundance in at least one sample. Note that these colours are not unique, but are cycled and the genera are plotted in order, so as e.g. Acaromyces (where present) is plotted at 0 and Other is always plotted at 100.
...
def get_stacked_bar(ra, ax, color_dict):
  ...
  for s in ra.columns:
    ...
    for g in genera:
      ...
      bottom += this_gen
    ax.bar(pltx[sn], 100, bottom=bottom, color=color_dict['Other (<2%)'], edgecolor='k', width=0.8)
...
plt.ylabel('Week 12\nRelative abundance (%)')
plt.xticks(all_x, all_samples, rotation=90, fontsize=8)
ax3.legend(handles=handles, loc='upper left', bbox_to_anchor=(-0.02,-0.2), ncol=8, fontsize=6)
plt.tight_layout()
plt.show()
```

```{python, echo=FALSE, results='hide', cache=TRUE}
def add_week(df, week):
  ind = list(df.index.values)
  ind_dict = {}
  sn = list(df.columns)
  sn_dict = {}
  ...
  for s in sn:
    sn_dict[s] = s.split('-')[0]
  df = df.rename(index=ind_dict, columns=sn_dict)
  return df
new_genus_W0 = add_week(pd.DataFrame(genus_W0), 'W0')
new_genus_W06 = add_week(pd.DataFrame(genus_W06), 'W06')
new_genus_W12 = add_week(pd.DataFrame(genus_W12), 'W12')

genus = pd.concat([new_genus_W0, new_genus_W06, new_genus_W12]).fillna(0)
genus = genus.round(1).sort_values(by=['name'])
```

### Fungi table
```{R, fungi_table, cache=TRUE, echo=FALSE}
genus = setDT(py$genus, keep.rownames = "Genus")[]
genus %>%
  mutate_if(is.numeric, function(x) {
    cell_spec(x, color=ifelse(x > 5, "white", "black"), bold=TRUE, background = spec_color(x, option = "A", end = 1, direction=-1))
  }) %>% 
...
### Stacked bar fungi (genus, overall abundance)

Here all fungi have again been grouped to the genus level and this again shows all genera that are above 2% relative abundance (as calculated above), but the values shown are normalised to the number of reads in each sample (reads per million). Note that these colours are again not unique (but are the same as above), but are cycled and the genera are plotted in order, so as e.g. Acaromyces (where present) is plotted at 0 and Other is always plotted at the top.

```{python, all_genus_aba, results='hide', fig.keep='all', cache=TRUE, echo=FALSE}
fungi_ra_W0 = pd.read_csv('/Users/robynwright/Documents/OneDrive/Langille Lab postdoc/fungal_metagenome/summary_files/W0_bracken_fungi.csv', header=0, index_col=0)
fungi_ra_W06 = pd.read_csv('/Users/robynwright/Documents/OneDrive/Langille Lab postdoc/fungal_metagenome/summary_files/W06_bracken_fungi.csv', header=0, index_col=0)
fungi_ra_W12 = pd.read_csv('/Users/robynwright/Documents/OneDrive/Langille Lab postdoc/fungal_metagenome/summary_files/W12_bracken_fungi.csv', header=0, index_col=0)

fungi_ra_W0 = fungi_ra_W0.divide(kraken_reads_W0.values).multiply(1000000)
fungi_ra_W06 = fungi_ra_W06.divide(kraken_reads_W06.values).multiply(1000000)
fungi_ra_W12 = fungi_ra_W12.divide(kraken_reads_W12.values).multiply(1000000)
  ...
  for s in ra.columns:
    ...
    for g in genera:
      ...
      if g not in all_genera:
        other.append(this_gen)
        continue
      ax.bar(pltx[sn], this_gen, bottom=bottom, color=color_dict[g], edgecolor='k', width=0.8)
      bottom += this_gen
    ax.bar(pltx[sn], other, bottom=bottom, color=color_dict['Other (<2%)'], edgecolor='k', width=0.8)
  ax.set_xlim([-0.7, all_x[-1]+0.5])
  plt.sca(ax)
  plt.semilogy()
  return
...
def add_week(df, week):
  ...
  for i in ind:
    if i in all_genera:
      keeping.append(True), not_keeping.append(False)
    else:
      keeping.append(False), not_keeping.append(True)
  all_keeping = pd.DataFrame(df.loc[keeping, :])
  all_others = pd.DataFrame(df.loc[not_keeping, :])
  all_others = pd.DataFrame(all_others.sum(axis=0)).transpose().rename(index={0:'Other <2%'})
  df = pd.concat([all_keeping, all_others])
  ind = list(df.index.values)
  ind_dict = {}
  sn = list(df.columns)
  sn_dict = {}
...
genus = pd.concat([new_genus_W0, new_genus_W06, new_genus_W12]).fillna(0)
genus = genus.round(1).sort_index(axis=0)
print(genus)
```

### Fungi table (overall abundance)
```{R, fungi_table_aba, cache=TRUE, echo=FALSE}
genus = setDT(py$genus, keep.rownames = "Genus")[]
genus %>%
  mutate_if(is.numeric, function(x) {
    cell_spec(x, color=ifelse(x > 5, "white", "black"), bold=TRUE, background = spec_color(x, option = "A", end = 1, direction=-1))
...
pway = '/Users/robynwright/Documents/OneDrive/Langille Lab postdoc/fungal_metagenome/humann_out/'
W0_genes = pd.read_csv(pway+'W0/humann_genefamilies_cpm.tsv', header=0, index_col=0, sep='\t')
W0_ko50 = pd.read_csv(pway+'W0/humann_genefamilies_cpm_ko50.tsv', header=0, index_col=0, sep='\t')
W0_ko90 = pd.read_csv(pway+'W0/humann_genefamilies_cpm_ko90.tsv', header=0, index_col=0, sep='\t')
W0_pathway = pd.read_csv(pway+'W0/humann_pathabundance_relab.tsv', header=0, index_col=0, sep='\t')

W06_genes = pd.read_csv(pway+'W06/humann_genefamilies_cpm.tsv', header=0, index_col=0, sep='\t')
W06_ko50 = pd.read_csv(pway+'W06/humann_genefamilies_cpm_ko50.tsv', header=0, index_col=0, sep='\t')
W06_ko90 = pd.read_csv(pway+'W06/humann_genefamilies_cpm_ko90.tsv', header=0, index_col=0, sep='\t')
W06_pathway = pd.read_csv(pway+'W06/humann_pathabundance_relab.tsv', header=0, index_col=0, sep='\t')

W12_genes = pd.read_csv(pway+'W12/humann_genefamilies_cpm.tsv', header=0, index_col=0, sep='\t')
W12_ko50 = pd.read_csv(pway+'W12/humann_genefamilies_cpm_ko50.tsv', header=0, index_col=0, sep='\t')
W12_ko90 = pd.read_csv(pway+'W12/humann_genefamilies_cpm_ko90.tsv', header=0, index_col=0, sep='\t')
W12_pathway = pd.read_csv(pway+'W12/humann_pathabundance_relab.tsv', header=0, index_col=0, sep='\t')

dropping = [W0_ko50, W0_ko90, W0_genes, W0_pathway, W06_ko50, W06_ko90, W06_genes, W06_pathway, W12_ko50, W12_ko90, W12_genes, W12_pathway]
for a in range(len(dropping)):
  drop = []
  for b in dropping[a].index.values:
    if 'unclassified' in b:
      drop.append(False)
    else:
      drop.append(True)
  dropping[a] = dropping[a].loc[drop, :]
W0_ko50, W0_ko90, W0_genes, W0_pathway, W06_ko50, W06_ko90, W06_genes, W06_pathway, W12_ko50, W12_ko90, W12_genes, W12_pathway = dropping

def combine(ko50, ko90):
  combined = pd.concat([ko50, ko90])
  combined = combined.drop(['UNGROUPED', 'UNMAPPED'], axis=0)
  combined = combined.groupby(combined.index.values).sum()
  combined = combined.append(1000000-combined.sum(axis=0), ignore_index=True)
  combined = combined.rename(index={combined.index.values[-1]:'Ungrouped'})
  return combined
  
W0_ko = combine(W0_ko50, W0_ko90)
W06_ko = combine(W06_ko50, W06_ko90)
W12_ko = combine(W12_ko50, W12_ko90)

W0_genes.drop(['UNMAPPED'], axis=0, inplace=True)
W06_genes.drop(['UNMAPPED'], axis=0, inplace=True)
W12_genes.drop(['UNMAPPED'], axis=0, inplace=True)
W0_pathway.drop(['UNMAPPED'], axis=0, inplace=True)
W06_pathway.drop(['UNMAPPED'], axis=0, inplace=True)
W12_pathway.drop(['UNMAPPED'], axis=0, inplace=True)
W0_ko.drop(['Ungrouped'], axis=0, inplace=True)
W06_ko.drop(['Ungrouped'], axis=0, inplace=True)
W12_ko.drop(['Ungrouped'], axis=0, inplace=True)

```

```{python, echo=FALSE}
def get_richness_plot(data):
  richness = []
  x = []
  for s in range(len(all_samples)):
    x.append(s)
    for sam in data.columns:
      if all_samples[s] in sam:
        one_sample = pd.DataFrame(data.loc[:, sam])
        one_sample = one_sample[one_sample.max(axis=1) > 0]
        richness.append(one_sample.shape[0])
  plt.figure(figsize=(10,4))
  ax1 = plt.subplot(111)
  for a in range(len(richness)):
    ax1.bar(x[a], richness[a], width=0.8, color='#8E0145', edgecolor='k')
  plt.xticks(x, all_samples, rotation=90, fontsize=8)
  plt.xlim([-0.7, x[-1]+0.5])
  if max(richness) > 10:
    plt.semilogy()
...
**W0:**
```{python, echo=FALSE}
get_richness_plot(W0_genes)
plt.show()
```

**W06:**
```{python, echo=FALSE}
get_richness_plot(W06_genes)
plt.show()
```

**W12:**
```{python, echo=FALSE}
get_richness_plot(W12_genes)
plt.show()
```
...
**W0:**
```{python, echo=FALSE}
get_richness_plot(W0_ko)
plt.show()
```

**W06:**
```{python, echo=FALSE}
get_richness_plot(W06_ko)
plt.show()
```

**W12:**
```{python, echo=FALSE}
get_richness_plot(W12_ko)
plt.show()
```
...
**W0:**
```{python, echo=FALSE}
get_richness_plot(W0_pathway)
plt.show()
```

**W06:**
```{python, echo=FALSE}
get_richness_plot(W06_pathway)
plt.show()
```

**W12:**
```{python, echo=FALSE}
get_richness_plot(W12_pathway)
plt.show()
```