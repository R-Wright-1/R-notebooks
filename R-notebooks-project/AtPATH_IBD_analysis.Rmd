...
folder = '/Users/robynwright/Documents/OneDrive/Langille_Lab_postdoc/AtPATH_IBD/'
```
...
I'll focus on CLR-transformation and removing samples with below 2500 reads here.

Import data:
```{R}
# metadata_fn = paste(py$save_path, 'metadata_reduced_ft.csv', sep='')
# ft_fn = paste(py$save_path, 'ft_2_reduced_metadata.csv', sep='')

asv_table <- read.csv(paste(py$save_path, 'ft_2_reduced_metadata.csv', sep='')) #read in table from file
sampledata <- read.csv(paste(py$save_path, 'metadata_reduced_ft_ccsplit.csv', sep='')) #read in the metadata table
phy_tree <- read_tree(paste(folder, "exports/tree.nwk", sep='')) #read in the phylogenetic tree

taxonomy <- asv_table[, c(1, 971)]
...
asv_table = asv_table[, 1:970]
samples = colnames(asv_table)[2:970]
for (s in 1:length(samples)) {
  samples[s] = substring(samples[s], 2)
  samples[s] = gsub("\\.", "-", samples[s])
}

asv_table_num = data.matrix(asv_table) #convert the ASV table to a numeric matric
rownames(asv_table_num) = asv_table[,1] #give the matrix row names
asv_table = as.matrix(asv_table_num) #convert it to a matrix
colnames(asv_table)[2:970] = samples

samples <- sampledata[, 2:719] #get the metadata columns
...
Remove samples with below 2500 reads, removing features that are only present as singletons, have fewer than 10 reads in total or are present in fewer than 5% of samples and CLR-normalise:
```{R}
physeq_red = prune_samples(sample_sums(physeq)>=2500, physeq)
physeq_red = phyloseq::filter_taxa(physeq_red, function(x) max(x) > 1, TRUE)
physeq_red = phyloseq::filter_taxa(physeq_red, function(x) sum(x) > 10, TRUE)
physeq_red = phyloseq_filter_prevalence(physeq_red, prev.trh = 0.05)

physeq_clr <- microbiome::transform(physeq_red, "clr") #convert to CLR
```

## Ordination {.tabset}

### Aitchison distance ASV (Euclidean on CLR)

```{R, fig.keep='all', message=FALSE}
options(warn = -1) 
ps.ord.ait <- ordinate(physeq_clr, "PCoA", "euclidean")
plot_ordination(physeq_clr, ps.ord.ait, type="samples", color="CASE_CONTROL", shape="GROUP") 
```

Adonis tests:
```{R}
distance <- phyloseq::distance(physeq_clr, method="euclidean", weighted=F)
print(adonis(distance ~ sample_data(physeq_clr)$CASE_CONTROL))
print(adonis(distance ~ sample_data(physeq_clr)$GROUP))
print(adonis(distance ~ sample_data(physeq_clr)$EXTRACTION_NUMBER))
```

### PHILR ASV

```{R, fig.keep='all', message=FALSE}
physeq_philr = physeq_red
...
ps.ord.philr <- ordinate(physeq_philr, 'PCoA', distance=philr.dist)
plot_ordination(physeq_philr, ps.ord.philr, type="samples", color="CASE_CONTROL", shape="GROUP") 
```

Adonis tests:
```{R}
print(adonis(philr.dist ~ sample_data(physeq_philr)$CASE_CONTROL))
print(adonis(philr.dist ~ sample_data(physeq_philr)$GROUP))
print(adonis(philr.dist ~ sample_data(physeq_philr)$EXTRACTION_NUMBER))
```

### Aitchison distance genus (Euclidean on CLR)

```{R, fig.keep='all', message=FALSE}
ps.rank = tax_glom(physeq_clr, taxrank="ta6", NArm=FALSE)
ps.ord.ait.gen <- ordinate(ps.rank, "PCoA", "euclidean")
plot_ordination(ps.rank, ps.ord.ait.gen, type="samples", color="CASE_CONTROL", shape="GROUP") 
```

Adonis tests:
```{R}
distance <- phyloseq::distance(ps.rank, method="euclidean", weighted=F)
print(adonis(distance ~ sample_data(ps.rank)$CASE_CONTROL))
print(adonis(distance ~ sample_data(ps.rank)$GROUP))
print(adonis(distance ~ sample_data(ps.rank)$EXTRACTION_NUMBER))
```

### PHILR genus

```{R, fig.keep='all', message=FALSE}
physeq_philr_rank = physeq_red
physeq_philr_rank <- transform_sample_counts(physeq_philr_rank, function(x) x+1)
physeq_philr_rank <- tax_glom(physeq_philr_rank, taxrank="ta6", NArm=FALSE)
phy_tree(physeq_philr_rank) <- makeNodeLabel(phy_tree(physeq_philr_rank), method="number", prefix='n')
otu.table <- t(otu_table(physeq_philr_rank))
tree <- phy_tree(physeq_philr_rank)
metadata <- sample_data(physeq_philr_rank)
tax <- tax_table(physeq_philr_rank)

physeq.philr.rank <- philr(otu.table, tree, part.weights='enorm.x.gm.counts', ilr.weights='blw.sqrt')
philr.dist.rank <- dist(physeq.philr.rank, method="euclidean")
ps.ord.philr.rank <- ordinate(physeq_philr_rank, 'PCoA', distance=philr.dist)
plot_ordination(physeq_philr_rank, ps.ord.philr.rank, type="samples", color="CASE_CONTROL", shape="GROUP") 
```

Adonis tests:
```{R}
print(adonis(philr.dist.rank ~ sample_data(physeq_philr_rank)$CASE_CONTROL))
print(adonis(philr.dist.rank ~ sample_data(physeq_philr_rank)$GROUP))
print(adonis(philr.dist.rank ~ sample_data(physeq_philr_rank)$EXTRACTION_NUMBER))
```

### RDA

```{R}
md_test <- c("SDC_AGE_CALC", "PM_STANDING_HEIGHT_AVG", "PM_WAIST_AVG", "PM_BIOIMPED_WEIGHT", "PM_WAIST_HIP_RATIO", "PM_BIOIMPED_FFM", "SALT_SEASONING", "NUTS_SEEDS_SERVINGS_PER_DAY", "NUT_VEG_DAY_QTY", "REFINED_GRAIN_SERVINGS_DAY_QTY", "SLE_LIGHT_EXP", "SDC_GENDER", "PM_BIOIMPED_BMI", "NUT_JUICE_DAY_QTY", "HS_DENTAL_VISIT_LAST", "CASE_CONTROL", "GROUP")
```

# Python

## Basic summary Python {.tabset}
