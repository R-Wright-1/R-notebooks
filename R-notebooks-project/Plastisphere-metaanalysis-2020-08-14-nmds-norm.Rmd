...
### Optional further diversity analyses 
...
### (I) Search for the top hits of the gene of interest in [uniprot](https://www.uniprot.org/)
...
### (IX) Get the heatmap using weighted and unweighted unifrac distances that shows the average distance within and between studies (Figure 3):
...
### (X) Get NMDS plots for the data with different transformations:
First reformat the raw file for R:
```{python, get_other_norm, results='hide', fig.keep='all', message=FALSE, cache=TRUE}
...
Now do the agglomeration and unifrac:
...
physeq_log <- transform_sample_counts(physeq_agglom, function (x) log1p(x+1))
...
asv_table_log <- read.csv(paste(py$basedir, "rarefy_comp/otu_table_log.csv", sep=""), sep=",", row.names=1)
asv_table_log = as.matrix(asv_table_log)
phy_tree_log <- read_tree(paste(py$basedir, "rarefy_comp/reduced_tree.tree", sep=""))
ASV_log <- otu_table(asv_table_log, taxa_are_rows = TRUE)
physeq_log <- phyloseq(ASV_log, phy_tree_log)
...
And then get the NMDS plots:
```{python, nmds_norm_test, results='hide', fig.keep='all', message=FALSE, cache=TRUE}
if not os.path.exists(basedir+'figures/nmds_log'+ext):
  w_uf_log, uw_uf_log = open_and_sort(basedir+'/rarefy_comp/weighted_unifrac_log.csv'), open_and_sort(basedir+ '/rarefy_comp/unweighted_unifrac_log.csv')
  w_uf_log.to_csv(basedir+'rarefy_comp/sorted_weighted_unifrac_log.csv')
  uw_uf_log.to_csv(basedir+'rarefy_comp/sorted_unweighted_unifrac_log.csv')
  #nmds_plot_study_env('/rarefy_comp/sorted_weighted_unifrac_log.csv', '/rarefy_comp/sorted_unweighted_unifrac_log.csv', meta_dict, basedir, save_name='figures/nmds_log')
  similarity_heatmap_combined(w_uf_log, uw_uf_log, basedir, save_name='figures/heatmap_log')

if not os.path.exists(basedir+'figures/nmds_clr'+ext):
  #w_uf_clr, uw_uf_clr = open_and_sort(basedir+'/rarefy_comp/weighted_unifrac_clr.csv'), open_and_sort(basedir+ '/rarefy_comp/unweighted_unifrac_clr.csv') 
  #w_uf_clr.to_csv(basedir+'rarefy_comp/sorted_weighted_unifrac_clr.csv')
  #uw_uf_clr.to_csv(basedir+'rarefy_comp/sorted_unweighted_unifrac_clr.csv')
  nmds_plot_study_env('/rarefy_comp/sorted_weighted_unifrac_clr.csv', '/rarefy_comp/sorted_weighted_unifrac_clr.csv', meta_dict, basedir, save_name='figures/nmds_clr')
  #similarity_heatmap_combined(w_uf_clr, uw_uf_clr, basedir, save_name='figures/heatmap_clr') 
```
...
### (XIII) Get the metacoder plots for early and late incubation times:
```{R, metacoder_r, results='hide', fig.keep='all', message=FALSE}
metacoder <- function(color1, color2) {
...
```{python, metacoder_plot, results='hide', fig.keep='all', message=FALSE, cache=TRUE, eval=FALSE}
...
metacoder(ft_df, tax_dict, meta_dict, basedir) 
```

### (XIV) Get the overall random forests:
Skip this if you downloaded the files of the already constructed random forests.
...
get_random_forests(ft_df, tax_dict, meta_df, basedir, est=est, n_jobs=n_jobs)
```

### (XV) Get the random forests for each environment for general plastic type:
```{python, get_rf_models_env, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
get_environment_random_forest(ft_df, tax_dict, meta_df, meta_dict, basedir, est=est, n_jobs=n_jobs) 
```

### (XVI) Get the random forests figures: {.tabset}
```{python, get_rf_plots_overall, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
get_overall_random_forest_plot(ft_df, meta_df, tax_dict, ASV_dict, meta_dict, basedir)
```
...
### (XVII) Get the environment random forest figures for general plastic type: {.tabset}
...
make_env_rf_plot(ft_df, tax_dict, basedir, ASV_dict, meta_dict)
make_env_rf_plot(ft_df, tax_dict, basedir, ASV_dict, meta_dict, mx=0.01)
```
...
### (XVIII) Get the PICRUSt2 plots (not included in the final manuscript):
```{python, picrust2_plots, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
KO_dict = open_pickle(KO_dict) #load the picrust kegg ortholog dictionary 
picrust = open_pickle(picrust) #and the picrust data
picrust_plots(picrust, KO_dict, meta_dict, basedir) #now make the picrust volcano and bar plot
```

And plot that figure:
