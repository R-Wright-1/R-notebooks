...
[Food or just a free ride? A meta-analysis reveals the global diversity of the Plastisphere]() (currently under revision for the ISME journal)
...
meta_fn, seqs, study_locs, study_dates, map_img = basedir+'metadata.txt', basedir+'qiime_output/dna-sequences_nr.fasta', basedir+'Study_location.csv', basedir+'Study_dates.csv', basedir+'world_map.jpg'
...
def nmds_plot_study_env(dist_matr_fn_w, dist_matr_fn_uw, meta_dict, basedir, n_jobs=1, save_name='/figures/Fig2_nmds_overall', stats=False):
    '''
    ...
    for z in [0, 1]:
        ...
        npos, handles = get_single_nmds(dist_matr, filter_on, filter_index, envs, env_index, ax[z][0], 'upper left', color_env, names, meta_dict, second_filter, second_filter_ind, '', n_jobs=n_jobs, get_stats=stats[z][0])
        if z == 0:
            plt.sca(ax[z][0])
            ...
            plt.sca(ax[z][0])
            plt.legend(handles=handles, loc='upper right', fontsize=fs_main, edgecolor='k')
        npos, handles = get_single_nmds(dist_matr, filter_on, filter_index, study, study_index, ax[z][1], 'upper right', color_study, names, meta_dict, second_filter, second_filter_ind, npos, n_jobs=n_jobs, get_stats=stats[z][1])
        if z == 0:
            plt.sca(ax[z][1])
...
def metacoder_py(ft, tax_dict, meta_dict, basedir):
    '''
    ...
    for a in range(len(envs)): #for each environment
        for z in range(len(comps)): #and each set of comparisons
            ...
            for b in range(len(comparisons)): #for each of our comparisons
                if os.path.exists(basedir+'/figures/metacoder/'+envs[a]+'_'+comparisons[b][0]+'_'+comparisons[b][1]+'_labels.pdf'):
                  if os.path.exists(basedir+'/figures/metacoder/'+envs[a]+'_'+comparisons[b][0]+'_'+comparisons[b][1]+'.pdf'):
                    print('Already have '+basedir+'/figures/metacoder/'+envs[a]+'_'+comparisons[b][0]+'_'+comparisons[b][1]+'.pdf')
                    continue
                if a not in [0, 3]:
                    continue
                ...
                r.suppressWarnings(r.metacoder(colors_metacoder))
                os.rename(basedir+'metacoder.pdf', basedir+'/figures/metacoder/'+envs[a]+'_'+comparisons[b][0]+'_'+comparisons[b][1]+'.pdf') #rename the first file, that doesn't have labels on the metacoder plot
                os.rename(basedir+'metacoder_labels.pdf', basedir+'/figures/metacoder/'+envs[a]+'_'+comparisons[b][0]+'_'+comparisons[b][1]+'_labels.pdf') #and then rename the second one, that does have labels on the metacoders plot
    return
...
def get_rf_comparison(basedir):
    ...
    plt.savefig(basedir+'/figures/FigS1_RF_compare'+ext, dpi=600, bbox_inches='tight')
    plt.close()
    return 
...
def get_single_forest(ft, meta_df, sn, level, est=10000, n_jobs=None):
    ...
    for a in range(len(meta_split)): #for each meta-category
        ...
        RF, RF_score, RF_importances = generate_rf(X_train, y_train, X_test, y_test, rc=reg_cls[a], est=est, n_jobs=None) #generate the random forest for this meta category
...
def get_random_forests(ft, tax_dict, meta_df, basedir, est=10000, n_jobs=None, sn=None, norm=None):
    ...
    for a in [7]:#[1, 2, 3, 4, 5, 6, 7]:
        ...
        if not os.path.exists(lev_sn+'.csv'): #only do this if we don't already have this file
          get_single_forest(this_ft, meta_df, lev_sn, a, est=est, n_jobs=None) #get all of the individual forests for this phylogenetic level
        else:
...
def get_heatmap_random_forest(df_rf, ft_in, colnames, basedir, level, other_levels, meta_name, predict, tax_dict, ASV_dict, other_folder=False, norm='clr'):
    '''
    ...
    plot_colorbar(ax4, 'viridis', merge_plot, orientation='vertical', name='CLR abundance', norm=norm)
    ...
    if other_folder == False:
        ...
        plt.savefig(basedir+'/figures/random_forest/single_category/'+phylo_level_names[level]+'_'+meta_name+'_single_forest'+ext, dpi=dpi, bbox_inches='tight')
    else: 
...
def get_random_forest_plots(ft, tax_dict, ASV_dict, meta_dict, basedir, other_folder=False, skip_individual=False, norm='clr'):
    '''
    ...
    for a in [1, 2, 3, 4, 5, 6, 7]:
        ...
        if other_folder == False:
            ...
            plt.xticks([])
            plt.yticks([])
            df_rf_sorted.loc[:, plot_labels]
            scores = scores.loc[:, plot_labels]
    ...
    if other_folder == False:
        plt.savefig(basedir+'/figures/random_forest/overall_forest_scores'+ext, dpi=dpi, bbox_inches='tight')
    else:
        of = other_folder.split('_')
        if of[0] in rename_plots:
            of[0] = rename_plots[of[0]]
...
def get_environment_random_forest_plots(ft, meta_df, tax_dict, ASV_dict, meta_dict, basedir, norm='clr'):
    '''
    ...
    for a in range(len(envs)):
        for b in range(len(labfield)):
            ...
            if len(single_meta_df.index.values) == 0:
                continue
            name = envs[a]
            scores = get_random_forest_plots(single_ft, tax_dict, ASV_dict, meta_dict, basedir, other_folder=name, skip_individual=True, norm=norm)
...
def get_overall_random_forest_plot(ft, meta_df, tax_dict, ASV_dict, meta_dict, basedir, norm='clr'):
    '''
    ...
    plt.savefig(basedir+'/figures/Fig5_random_forest'+ext, dpi=dpi, bbox_inches='tight')
    return
```
...
R:
```{R, R_input_functions, results='hide', fig.keep='all', message=FALSE}
...
metacoder <- function(colors_metacoder) {
...
Sort these so that they are all made at once and check if they are already made:
### (IV) Perform agglomeration and calculate unifrac distances:
```{R, agglom_unifrac, results='hide', fig.keep='all', cache=TRUE, message=FALSE, eval=FALSE}
...
#And get the unifrac distances for only the CLR transformed data (as that is what we will show)
...
Changing the parts that are between the ''''''' will allow you to use a different normalisation method for all further analyses/plots.
...
tree_agglom = basedir+'/agglom/reduced_tree_nr.tree'
tax_dict_rare = open_pickle(basedir+'tax_dict_rare.dictionary')
tax_dict_nr = open_pickle(basedir+'tax_dict_nr.dictionary')
tax_matrix = pd.read_csv(basedir+'tax_dict_nr.csv')

''''''
ft_df = ft_df_clr
ft_full = basedir+'/feature_table_nr.csv'
tax_dict = tax_dict_nr
...
tax_matrix = tax_matrix.loc[ft_df.index.values, :]
tax_matrix.drop(['Species name'], axis=1, inplace=True)
tax_matrix = tax_matrix.reset_index()
```

### (VII) Get PCA plot using Aitchison distance
```{R}
asv_table <- py$ft_df_nr
asv_table = as.matrix(asv_table)
asv_table_clr <- py$ft_df_clr
asv_table_clr = as.matrix(asv_table_clr)
...
#Convert these to phyloseq objects
ASV = otu_table(asv_table, taxa_are_rows = TRUE)
ASV_clr = otu_table(asv_table_clr, taxa_are_rows = TRUE)
TAX = tax_table(taxmat2)
taxa_names(TAX) <- taxmat[,1]
physeq = phyloseq(ASV,phy_tree,TAX, sampledata)
physeq_clr = phyloseq(ASV_clr,phy_tree,TAX, sampledata)
...
#now the distance plot
physeq.dist <- dist(physeq.philr, method="euclidean")
physeq.pcoa <- ordinate(physeq, 'PCoA', distance=physeq.dist)
plot_ordination(physeq, physeq.pcoa, color='Study') + geom_point(size=1)

physeq.dist.aitch.clr <- dist(physeq_clr, method="euclidean")
physeq.pcoa.aitch <- ordinate(physeq_clr, 'PCoA', distance=physeq.dist.aitch.clr)
plot_ordination(physeq_clr, physeq.pcoa.aitch, color='Study') + geom_point(size=1)
```


### (VII) Get the NMDS plots using weighted and unweighted unifrac distances (Figure 2):
...
if os.path.exists(basedir+'anosim_permanova.list'):
  stats = open_pickle(basedir+'anosim_permanova.list')
  run_tests = False
else:
  group_env, group_study = get_stats_groups(uw_uf, meta_dict)
  run_tests = True
```

Carry out the stats tests:
```{R, perm_anosim, results='hide', fig.keep='all', message=FALSE, cache=TRUE}

if (!file.exists(paste(py$basedir, "anosim_permanova.list", sep=""))) {
  as_uw_study <- anosim(py$uw_uf, py$group_study, permutations=999)
  as_uw_env <- anosim(py$uw_uf, py$group_env, permutations=999)
  as_w_study <- anosim(py$w_uf, py$group_study, permutations=999)
  as_w_env <- anosim(py$w_uf, py$group_env, permutations=999)
  
  as_uw_study <- c(as_uw_study$statistic, as_uw_study$signif)
  as_uw_env <- c(as_uw_env$statistic, as_uw_env$signif)
  as_w_study <- c(as_w_study$statistic, as_w_study$signif)
  as_w_env <- c(as_w_env$statistic, as_w_env$signif)
  
  perm_uw_study <- adonis(t(py$uw_uf) ~ py$group_study, data=py$uw_uf)
  perm_uw_env <- adonis(t(py$uw_uf) ~ py$group_env, data=py$uw_uf)
  perm_w_study <- adonis(t(py$w_uf) ~ py$group_study, data=py$w_uf)
  perm_w_env <- adonis(t(py$w_uf) ~ py$group_env, data=py$w_uf)
  
  perm_uw_study <- c(perm_uw_study$aov.tab$F.Model[1], perm_uw_study$aov.tab$`Pr(>F)`[1])
  perm_uw_env <- c(perm_uw_env$aov.tab$F.Model[1], perm_uw_env$aov.tab$`Pr(>F)`[1])
  perm_w_study <- c(perm_w_study$aov.tab$F.Model[1], perm_w_study$aov.tab$`Pr(>F)`[1])
  perm_w_env <- c(perm_w_env$aov.tab$F.Model[1], perm_w_env$aov.tab$`Pr(>F)`[1])
}
```
These were previously run in Python, but seemed to have trouble making that work consistently - they would only run for the weighted and threw a distance matrix (scikit-bio) error (saying that it either wasn't symmetrical or had NaNs - neither of which were true) for the unweighted. The weighted statistics were the same for both, though

And then plot the NMDS:
```{python, uf_nmds, results='hide', fig.keep='all', message=FALSE, cache=TRUE}
if run_tests:
  stats = [[r.as_w_env+r.perm_w_env, r.as_w_study+r.perm_w_study], [r.as_uw_env+r.perm_uw_env, r.as_uw_study+r.perm_uw_study]]

if not os.path.exists(basedir+'/figures/Fig2_nmds_overall'+ext):
  nmds_plot_study_env('agglom/sorted_weighted_unifrac.csv', 'agglom/sorted_unweighted_unifrac.csv', meta_dict, basedir, stats=stats)

```

This should save Figure 2 as 'figures/Fig2_nmds_overall.png' (if you have changed the extension then this will be different):
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/Fig2_nmds_overall.png)

### (VIII) Get the heatmap using weighted and unweighted unifrac distances that shows the average distance within and between studies (Figure 3):
...
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/Fig3_unifrac_heatmap_combined.png)
...
if not os.path.exists(basedir+'/figures/Fig4_dendro_venn'+ext):
...
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/Fig4_dendro_venn.png)
...
```{python, get_rf_models_overall, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
get_random_forests(ft_df_rare, tax_dict, meta_df, basedir, est=est, n_jobs=n_jobs, norm='rare')
get_random_forests(ft_df_log, tax_dict_other_norm, meta_df, basedir, est=est, n_jobs=n_jobs, norm='log')
get_random_forests(ft_df_clr, tax_dict_other_norm, meta_df, basedir, est=est, n_jobs=n_jobs, norm='clr')
get_random_forests(ft_df_rel_abun, tax_dict_other_norm, meta_df, basedir, est=est, n_jobs=n_jobs, norm='rel_abun')
```

### (XI) Get the random forests for each environment for general plastic type:
```{python, get_rf_models_env, results='hide', fig.keep='all', message=FALSE, cache=TRUE, eval=FALSE}
#add norm name to these
get_environment_random_forest(ft_df_rare, tax_dict, meta_df, meta_dict, basedir, est=est, n_jobs=n_jobs, norm='rare') 
get_environment_random_forest(ft_df_log, tax_dict_other_norm, meta_df, meta_dict, basedir, est=est, n_jobs=n_jobs, norm='log') 
get_environment_random_forest(ft_df_clr, tax_dict_other_norm, meta_df, meta_dict, basedir, est=est, n_jobs=n_jobs, norm='clr') 
get_environment_random_forest(ft_df_rel_abun, tax_dict_other_norm, meta_df, meta_dict, basedir, est=est, n_jobs=n_jobs, norm='rel_abun') 
```
...
if not os.path.exists(basedir+'figures/FigS1_RF_compare'+ext):
  get_rf_comparison(basedir)
```

This should save Figure S1 as 'figures/FigS1_RF_comp.png' (if you have changed the extension then this will be different):
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/FigS1_RF_compare.png)

### (XIII) Get the random forest figures: {.tabset}
Need to rerun after ASV has finished running
```{python, get_rf_plots_overall, results='hide', fig.keep='all', message=FALSE, cache=TRUE}
get_overall_random_forest_plot(ft_df, meta_df, tax_dict, ASV_dict, meta_dict, basedir, norm='clr')
```
...
```{python, get_rf_plots_separate, results='hide', fig.keep='all', message=FALSE, cache=TRUE, eval=FALSE}
get_random_forest_plots(ft_df, tax_dict, ASV_dict, meta_dict, basedir, norm='clr')
```

And now plot plot those figures here:

#### All metadata categories {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/phyla_overall_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/classes_overall_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/orders_overall_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/families_overall_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/genera_overall_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/species_overall_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/ASVs_overall_forest.png)

#### Depth {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_DEPTH_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_DEPTH_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_DEPTH_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_DEPTH_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_DEPTH_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_DEPTH_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_DEPTH_single_forest.png)

#### DNA extraction method {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_DNAExtraction_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_DNAExtraction_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_DNAExtraction_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_DNAExtraction_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_DNAExtraction_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_DNAExtraction_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_DNAExtraction_single_forest.png)

#### Environment {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Environment_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Environment_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Environment_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Environment_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Environment_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Environment_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Environment_single_forest.png)

#### General incubation time {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_IncubationGeneral_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_IncubationGeneral_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_IncubationGeneral_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_IncubationGeneral_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_IncubationGeneral_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_IncubationGeneral_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_IncubationGeneral_single_forest.png)

#### Specific incubation time {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_IncubationTime_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_IncubationTime_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_IncubationTime_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_IncubationTime_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_IncubationTime_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_IncubationTime_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_IncubationTime_single_forest.png)

#### Incubation or collection {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_IncubationOrCollection_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_IncubationOrCollection_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_IncubationOrCollection_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_IncubationOrCollection_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_IncubationOrCollection_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_IncubationOrCollection_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_IncubationOrCollection_single_forest.png)

#### Lab or Field {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_LabOrField_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_LabOrField_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_LabOrField_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_LabOrField_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_LabOrField_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_LabOrField_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_LabOrField_single_forest.png)

#### Latitude {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Latitude_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Latitude_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Latitude_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Latitude_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Latitude_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Latitude_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Latitude_single_forest.png)

#### Longitude {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Longitude_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Longitude_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Longitude_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Longitude_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Longitude_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Longitude_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Longitude_single_forest.png)

#### Light {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Light_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Light_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Light_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Light_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Light_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Light_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Light_single_forest.png)

#### Material type {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_MaterialType_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_MaterialType_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_MaterialType_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_MaterialType_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_MaterialType_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_MaterialType_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_MaterialType_single_forest.png)

#### General plastic type {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_PlasticTypeGeneral_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_PlasticTypeGeneral_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_PlasticTypeGeneral_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_PlasticTypeGeneral_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_PlasticTypeGeneral_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_PlasticTypeGeneral_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_PlasticTypeGeneral_single_forest.png)

#### Specific plastic type {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_PlasticTypeSpecific_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_PlasticTypeSpecific_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_PlasticTypeSpecific_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_PlasticTypeSpecific_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_PlasticTypeSpecific_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_PlasticTypeSpecific_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_PlasticTypeSpecific_single_forest.png)

#### Primer pair {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_PrimerPair_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_PrimerPair_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_PrimerPair_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_PrimerPair_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_PrimerPair_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_PrimerPair_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_PrimerPair_single_forest.png)

#### Salinity {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Salinity_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Salinity_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Salinity_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Salinity_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Salinity_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Salinity_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Salinity_single_forest.png)

#### Season {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Season_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Season_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Season_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Season_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Season_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Season_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Season_single_forest.png)

#### Source {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Source_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Source_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Source_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Source_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Source_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Source_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Source_single_forest.png)

#### Study {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Study_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Study_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Study_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Study_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Study_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Study_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Study_single_forest.png)

#### Temperature {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_Temperature_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_Temperature_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_Temperature_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_Temperature_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_Temperature_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_Temperature_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_Temperature_single_forest.png)

#### Water or sediment {.tabset}

##### Phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/phyla_WaterOrSediment_single_forest.png)

##### Class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/classes_WaterOrSediment_single_forest.png)

##### Order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/orders_WaterOrSediment_single_forest.png)

##### Family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/families_WaterOrSediment_single_forest.png)

##### Genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/genera_WaterOrSediment_single_forest.png)

##### Species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/species_WaterOrSediment_single_forest.png)

##### ASV
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/random_forest/single_category/ASVs_WaterOrSediment_single_forest.png)

### (XIV) Get the random forest plots comparing the different normalisation methods for the random forests for each environment on plastic type:
...
```{python, get_rf_plots_env, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
...
def make_env_rf_plot(ft, tax_dict, basedir, ASV_dict, meta_dict, mx=0.005, norm='clr'):
    '''
...
ft_df_nr = open_and_sort(basedir+'/agglom/otu_table_nr.csv')
get_environment_random_forest_plots(ft_df, meta_df, tax_dict, ASV_dict, meta_dict, basedir)
make_env_rf_plot(ft_df_nr, tax_dict, basedir, ASV_dict, meta_dict)
make_env_rf_plot(ft_df_nr, tax_dict, basedir, ASV_dict, meta_dict, mx=0.01)
```

And now plot those figures:

#### Feature importance > 0.005
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/environment_random_forest_0.005.png)

#### Feature importance > 0.01
![]()

### (XV) Get the metacoder plots for early and late incubation times:
```{python, metacoder_plot, results='hide', fig.keep='all', message=FALSE, cache=TRUE}
ft_df_ra = pd.DataFrame(ft_df_rel_abun)
metacoder_py(ft_df_ra, tax_dict, meta_dict, basedir)
```
...
path_to_pdf = basedir+'figures/metacoder/'
...
Make a colorbar:
```{python, metacoder_colorbar, results='hide', fig.keep='all', message=FALSE, cache=TRUE}
...
##### Aliphatic - early *vs* late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/marine_aliphatic early_aliphatic late_labels.png)

##### Other plastic - early *vs* late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_other plastic early_other plastic late_labels.png)

##### Biofilm - early *vs* late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_biofilm early_biofilm late_labels.png)

##### Early - aliphatic *vs* other plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic early_biofilm early_labels.png)

##### Early - aliphatic *vs* unknown plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic early_unknown plastic early_labels.png)

##### Early - other plastic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_other plastic early_biofilm early_labels.png)

##### Early - other plastic *vs* unknown plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_other plastic early_unknown plastic early_labels.png)

##### Early - unknown plastic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_unknown plastic early_biofilm early_labels.png)

##### Late - aliphatic *vs* other plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic late_other plastic late_labels.png)

##### Late - aliphatic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic late_biofilm late_labels.png)

##### Late - other plastic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_other plastic late_biofilm late_labels.png)

##### Aliphatic *vs* other plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic_other plastic_labels.png)

##### Aliphatic *vs* unknown plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic_unknown plastic_labels.png)

##### Aliphatic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic_biofilm_labels.png)

##### Aliphatic *vs* planktonic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_aliphatic_planktonic_labels.png)

##### Other plastic *vs* unknown plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_other plastic_unknown plastic_labels.png)

##### Other plastic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_other plastic_biofilm_labels.png)

##### Other plastic *vs* planktonic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_other plastic_planktonic_labels.png)

##### Unknown plastic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_unknown plastic_biofilm_labels.png)

##### Unknown plastic *vs* planktonic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_unknown plastic_planktonic_labels.png)

##### Biofilm *vs* planktonic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/marine_biofilm_planktonic_labels.png)

#### Aquatic {.tabset}

##### Aliphatic - early *vs* late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/aquatic_aliphatic early_aliphatic late_labels.png)

##### Other plastic - early *vs late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/aquatic_other plastic early_other plastic late_labels.png)

##### Biofilm - early *vs* late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/aquatic_biofilm early_biofilm late_labels.png)

##### Early - aliphatic *vs* other plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/aquatic_aliphatic early_other plastic early_labels.png)

##### Aliphatic *vs* other plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/aquatic_aliphatic_other plastic_labels.png)

##### Aliphatic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/aquatic_aliphatic_biofilm_labels.png)

##### Aliphatic *vs* planktonic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/aquatic_aliphatic_planktonic_labels.png)

#### Freshwater {.tabset}

##### Other plastic - early *vs* late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/freshwater_other plastic early_other plastic late_labels.png)

##### Biofilm - early *vs* late
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/freshwater_biofilm early_biofilm late_labels.png)

##### Early - aliphatic *vs* other plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/freshwater_aliphatic early_other plastic early_labels.png)

##### Aliphatic *vs* other plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/freshwater_aliphatic_other plastic_labels.png)

##### Aliphatic *vs* unknown plastic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/freshwater_aliphatic_unknown plastic_labels.png)

##### Aliphatic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/freshwater_aliphatic_biofilm_labels.png)

##### Aliphatic *vs* planktonic
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/freshwater_aliphatic_planktonic_labels.png)

#### Terrestrial {.tabset}

##### Unknown plastic *vs* biofilm
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/paper_data/figures/metacoder/Labels/terrestrial_unknown plastic_biofilm_labels.png)


Still working on:

### (XIX) Get the separate plots for each study: {.tabset}
```{python, separate_studies, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
plots_per_study(ft_df, meta_df, meta_dict, w_uf, uw_uf, tax_dict, ASV_dict, basedir, est=est, n_jobs=n_jobs) #get the plots summarising the results per study
```

And now plot them here:

#### First one

#### etc

### (XII) Get Ancom trees and heatmaps for early and late incubation times: {.tabset}
*Consider coming back to - it is not actually used, but maybe I shouldn't remove it?*
I need to come back to this section to make it work. For some reason the function will work when I just directly run it in R, but not when called from Python (even though there are no additional inputs from Python) - this gives the following warning that I have not been successful in fixing. But everything worked fine when I just called the R script using os.system('R location', 'R script'):
```{R, eval=FALSE}
RuntimeError: Evaluation error: Evaluation error: Unable to convert R object to Python type..

Detailed traceback: 
  File "<string>", line 1, in <module>
  File "<string>", line 126, in tree_heatmap
...
```
```{R, ancom_tree, results='hide', fig.keep='all', message=FALSE}
...
#load_all_R_packages()
```
```{python, ancom_tree_heatmap, results='hide', fig.keep='all', message=FALSE, cache=TRUE, eval=FALSE}
...
def tree_heatmap(ft, meta_dict, basedir, tax_dict, level=7):
    ...
    for a in range(len(envs)): #for each environment
        ...
        if sum(all_comps) > 0: #if we had results for some competitions
            ...
            os.rename('ancom_heatmap.pdf', basedir+'/figures/ancom/'+envs[a]+'_ancom_significance_'+lvls[level]+'.pdf') #rename the output of the R script
    ...
    return
#[tree_heatmap(ft_df, meta_dict, basedir, tax_dict, level=al) for al in [1, 2, 3, 4, 5, 6]]
tree_heatmap(ft_df, meta_dict, basedir, tax_dict, level=1)
```
...
And then show all of these plots here:

#### Marine phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/marine_ancom_significance_phylum.png)

#### Marine class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/marine_ancom_significance_class.png)

#### Marine order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/marine_ancom_significance_order.png)

#### Marine family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/marine_ancom_significance_family.png)

#### Marine genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/marine_ancom_significance_genus.png)

#### Marine species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/marine_ancom_significance_species.png)

#### Aquatic phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/aquatic_ancom_significance_phylum.png)

#### Aquatic class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/aquatic_ancom_significance_class.png)

#### Aquatic order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/aquatic_ancom_significance_order.png)

#### Aquatic family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/aquatic_ancom_significance_family.png)

#### Aquatic genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/aquatic_ancom_significance_genus.png)

#### Aquatic species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/aquatic_ancom_significance_species.png)

#### Freshwater phylum
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/freshwater_ancom_significance_phylum.png)

#### Freshwater class
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/freshwater_ancom_significance_class.png)

#### Freshwater order
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/freshwater_ancom_significance_order.png)

#### Freshwater family
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/freshwater_ancom_significance_family.png)

#### Freshwater genus
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/freshwater_ancom_significance_genus.png)

#### Freshwater species
![](/Users/robynwright/Documents/OneDrive/Papers_writing/Plastisphere_Meta-analysis/test_recreate_analyses/recreate_analyses/figures/ancom/freshwater_ancom_significance_species.png)
