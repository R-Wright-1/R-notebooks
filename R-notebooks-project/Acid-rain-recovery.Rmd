...
samples <- sampledata[, 2:6] #get the metadata columns
...
physeq_log2 <- transform_sample_counts(physeq, function(x) log2(x+1)) #convert to log2
```
...
```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
plot_richness(physeq, x="SoilHorizon", measures=c("Observed", "Chao1", "Simpson", "Shannon")) + geom_boxplot()
```
...
```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
plot_richness(physeq_rare, x="SoilHorizon", measures=c("Observed", "Chao1", "Simpson", "Shannon")) + geom_boxplot()
```

```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
plot_richness(physeq_rare, "SoilHorizon", "Treatment", measures=c("Observed", "Chao1", "Simpson", "Shannon")) + geom_boxplot()
```
...
print(adonis(distance ~ sample_data(ps)$SoilHorizon))
```
...
ps = physeq
ps.ord <- ordinate(ps, "PCoA", "bray")
plot_ordination(ps, ps.ord, type="samples", color="SoilHorizon", shape="Treatment") 
distance <- phyloseq::distance(ps, method="bray", weighted=F)
print(adonis(distance ~ sample_data(ps)$Treatment))
print(adonis(distance ~ sample_data(ps)$Location))
print(adonis(distance ~ sample_data(ps)$Location2))
...
Here the first 4 adonis tests are for weighted unifrac and the second 3 are for unweighed.
...
ps = physeq
ps.ord.w <- ordinate(ps, "PCoA", "unifrac", weighted=TRUE)
w <- plot_ordination(ps, ps.ord.w, type="samples", color="SoilHorizon", shape="Treatment", title="Weighted Unifrac")
ps.ord.uw <- ordinate(ps, "PCoA", "unifrac", weighted=FALSE)
uw <- plot_ordination(ps, ps.ord.uw, type="samples", color="SoilHorizon", shape="Treatment", title="Unweighted Unifrac") 
grid.arrange(w, uw, nrow = 1)
distance <- phyloseq::distance(ps, method="unifrac", weighted=T)
print(adonis(distance ~ sample_data(ps)$Treatment))
print(adonis(distance ~ sample_data(ps)$Location))
...
uw <- plot_ordination(ps, ps.ord.uw, type="samples", color="SoilHorizon", shape="Treatment", title="Unweighted Unifrac") 
grid.arrange(w, uw, nrow = 1)
w  = plot_ordination(ps, ps.ord.w, type="samples", color="SoilHorizon", shape="Treatment")
w + geom_point(size=3) + geom_polygon(aes(fill=SoilHorizon, alpha=0.1))
distance <- phyloseq::distance(ps, method="unifrac", weighted=T)
print(adonis(distance ~ sample_data(ps)$Treatment))
print(adonis(distance ~ sample_data(ps)$Location))
print(adonis(distance ~ sample_data(ps)$Location2))
...
print(adonis(philr.dist ~ sample_data(ps)$SoilHorizon))
```
...
### Relative abundance Treatment/Location {.tabset}
...
### Relative abundance Horizon/Treatment {.tabset}
...
plot_bar(ps.rank, fill=rnk) + facet_wrap(c(~SoilHorizon, ~Treatment), scales="free_x", nrow=1) + theme(legend.text=element_text(size=5), legend.key.size = unit(0.3, "cm")) + guides(fill=guide_legend(ncol=1)) + scale_fill_manual(values=palette)
```
...
### Rarefied Treatment/Location{.tabset}
...
### Rarefied Horizon/Treatment{.tabset}
...
plot_tree(ps_trim, color="SoilHorizon", label.tips = "ta5", ladderize="left", size="Abundance")
```
...
### Soil Horizon differential abundance {.tabset}

#### Forest Floor vs Humis Layer/Forest Floor

Calculate:
```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
s1 = "Forest Floor "
s2 = "Humis Layer/Forest Floor"
obj <- parse_tax_data(asv_tax, class_cols = "lineage", class_sep = ";") 
obj$data$tax_data <- calc_obs_props(obj, "tax_data")
obj$data$tax_abund <- calc_taxon_abund(obj, "tax_data")
obj$data$sample_data = x$data$sample_data
...
for (i in 2:length(cn)) {
  for (j in 1:length(rn)) {
    if (obj$data$sample_data$sample_id[j] == cn[i]) {
      if (obj$data$sample_data$SoilHorizon[j] == s1) {
        keeping = c(keeping, cn[i])
        group = c(group, obj$data$sample_data$SoilHorizon[j])
      } else if (obj$data$sample_data$SoilHorizon[j] == s2) {
        keeping = c(keeping, cn[i])
        group = c(group, obj$data$sample_data$SoilHorizon[j])
      }
    }
...
#### Forest Floor vs Upper B Horizon
...
#### Humis Layer/Forest Floor vs Upper B Horizon

Calculate:
```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
s1 = "Humis Layer/Forest Floor"
s2 = "Upper B Horizon"
obj <- parse_tax_data(asv_tax, class_cols = "lineage", class_sep = ";") 
obj$data$tax_data <- calc_obs_props(obj, "tax_data")
obj$data$tax_abund <- calc_taxon_abund(obj, "tax_data")
...
This didn't work and I think it was because there were no significant differences (which wouldn't be surprising because they didn't really look like they grouped separately).

```{R, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
options(warn = -1) 
samples_keeping = c("DNA10", "DNA13", "DNA16", "Batch2DNA1", "Batch2DNA4", "Batch2DNA7", "Batch2DNA10", "Batch2DNA13", "DNA8", "DNA11", "DNA14", "DNA17", "Batch2DNA2", "Batch2DNA5", "Batch2DNA8", "Batch2DNA11", "Batch2DNA14")
ps = prune_samples(samples_keeping, physeq)
sample_data(ps)$Treatment <- as.factor(sample_data(ps)$SoilHorizon)
ds = phyloseq_to_deseq2(ps, ~ SoilHorizon)
ds = DESeq(ds)
alpha = 0.05
res = results(ds, contrast=c("SoilHorizon", "Forest Floor ", "Humis Layer/Forest Floor"), alpha=alpha)
res = res[order(res$padj, na.last=NA), ]
res_sig = res[(res$padj < alpha), ]
res_sig = cbind(as(res_sig, "data.frame"), as(tax_table(ps)[rownames(res_sig), ], "matrix"))
ggplot(res_sig, aes(x=ta4, y=log2FoldChange, color=ta2)) +
...
So now we're getting quite a lot of significant differences in the abundance of taxa. 

```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
samples_keeping = c("DNA7", "DNA10", "DNA13", "DNA16", "Batch2DNA1", "Batch2DNA4", "Batch2DNA7", "Batch2DNA10", "Batch2DNA13", "DNA9", "DNA12", "DNA15", "DNA18", "Batch2DNA3", "Batch2DNA6", "Batch2DNA9", "Batch2DNA12", "Batch2DNA15")
ps = prune_samples(samples_keeping, physeq)
sample_data(ps)$Treatment <- as.factor(sample_data(ps)$SoilHorizon)
ds = phyloseq_to_deseq2(ps, ~ SoilHorizon)
ds = DESeq(ds)
alpha = 0.05
res = results(ds, contrast=c("SoilHorizon", "Forest Floor ", "Upper B Horizon"), alpha=alpha)
res = res[order(res$padj, na.last=NA), ]
res_sig = res[(res$padj < alpha), ]
res_sig = cbind(as(res_sig, "data.frame"), as(tax_table(ps)[rownames(res_sig), ], "matrix"))
ggplot(res_sig, aes(x=ta4, y=log2FoldChange, color=ta2)) +
...
Plot a tree with these significantly differentially abundant (DA) ASVs. Here we actually have 444 ASVs that are DA bewteen horizons. As this is too many to easily display on a tree, I'll just take the 35 with the highest and lowest log2 fold changes). This is plotted using the CLR abundance:
```{R, results='hide', fig.keep='all', fig.height=5, message=FALSE}
options(warn = -1) 
library(microbiome)
library(phyloseq)

ord_res_sig <- res_sig[order(res_sig$log2FoldChange),]
low = row.names(head(ord_res_sig, 35))
high = row.names(tail(ord_res_sig, 35))
asv_sig = c(low, high)

ps = prune_samples(samples_keeping, physeq_relabun)
ps = prune_taxa(asv_sig, ps)
tree = phy_tree(ps)
asv = otu_table(ps)
tax = tax_table(ps)
...
p <- ggtree(tree, layout="fan", open.angle=25)
p <- gheatmap(p, data_mean, colnames_angle=90, font.size=4, hjust=1, color="black", offset=0.7) + scale_fill_viridis_b()
p <- p %<+% d + geom_tiplab(aes(label=species), parse=F, size=2, align=T, linesize=.12) + guides(fill = guide_legend(title = "Relative abundance"))
p
```

### Humis Layer/Forest Floor vs Upper B Horizon

And again we have quite a lot of significant differences in the abundance of taxa. 

```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
samples_keeping = c("DNA8", "DNA11", "DNA14", "DNA17", "Batch2DNA2", "Batch2DNA5", "Batch2DNA8", "Batch2DNA11", "Batch2DNA14", "DNA9", "DNA12", "DNA15", "DNA18", "Batch2DNA3", "Batch2DNA6", "Batch2DNA9", "Batch2DNA12", "Batch2DNA15")
ps = prune_samples(samples_keeping, physeq)
sample_data(ps)$Treatment <- as.factor(sample_data(ps)$SoilHorizon)
ds = phyloseq_to_deseq2(ps, ~ SoilHorizon)
ds = DESeq(ds)
alpha = 0.05
res = results(ds, contrast=c("SoilHorizon", "Humis Layer/Forest Floor", "Upper B Horizon"), alpha=alpha)
res = res[order(res$padj, na.last=NA), ]
res_sig = res[(res$padj < alpha), ]
res_sig = cbind(as(res_sig, "data.frame"), as(tax_table(ps)[rownames(res_sig), ], "matrix"))
ggplot(res_sig, aes(x=ta4, y=log2FoldChange, color=ta2)) +
...
Plot a tree with these significantly differentially abundant (DA) ASVs. As above, we're showing the top 35 and lowest 35, as this time we have 576 DA taxa:
```{R, results='hide', fig.keep='all', fig.height=5, message=FALSE}
options(warn = -1) 
library(microbiome)
library(phyloseq)
...
write.csv(asv_tax, paste(folder, "Humis vs upper ASV.csv"))
...
p <- ggtree(tree, layout="fan", open.angle=35)
p <- gheatmap(p, data_mean, colnames_angle=90, font.size=4, hjust=1, color="black", offset=1) + scale_fill_viridis_b()
p <- p %<+% d + geom_tiplab(aes(label=species), parse=F, size=2, align=T, linesize=.12) + guides(fill = guide_legend(title = "Relative abundance"))
p
```

### Forest Floor: Control vs Treatment

Again, this didn't work and I think it's again because there aren't any significant differences.

```{R, results='hide', fig.keep='all', message=FALSE, eval=FALSE}
options(warn = -1) 
samples_keeping = c("DNA7", "DNA10", "DNA13", "DNA16", "Batch2DNA1", "Batch2DNA4", "Batch2DNA7", "Batch2DNA10", "Batch2DNA13")
ps = prune_samples(samples_keeping, physeq)
sample_data(ps)$Treatment <- as.factor(sample_data(ps)$Treatment)
ds = phyloseq_to_deseq2(ps, ~ Treatment)
ds = DESeq(ds)
...
### Humis Layer/Forest Floor: Control vs Treatment

So only one significant difference here.

```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
samples_keeping = c("DNA8", "DNA11", "DNA14", "DNA17", "Batch2DNA2", "Batch2DNA5", "Batch2DNA8", "Batch2DNA11", "Batch2DNA14")
ps = prune_samples(samples_keeping, physeq)
sample_data(ps)$Treatment <- as.factor(sample_data(ps)$Treatment)
ds = phyloseq_to_deseq2(ps, ~ Treatment)
ds = DESeq(ds)
...
### Upper B Horizon: Control vs Treatment

These ones looked like they grouped the most separately from one another on the beta diversity ordination plots, so not really surprising that this is where there are the most taxa that are significantly differentially abundant between treatments.

```{R, results='hide', fig.keep='all', message=FALSE}
options(warn = -1) 
samples_keeping = c("DNA9", "DNA12", "DNA15", "DNA18", "Batch2DNA3", "Batch2DNA6", "Batch2DNA9", "Batch2DNA12", "Batch2DNA15")
ps = prune_samples(samples_keeping, physeq)
sample_data(ps)$Treatment <- as.factor(sample_data(ps)$Treatment)
ds = phyloseq_to_deseq2(ps, ~ Treatment)
ds = DESeq(ds)
...
Plot a tree with these significantly differentially abundant (DA) ASVs. There are just 20 DA taxa here, so I'm plotting all of them on this tree:
```{R, results='hide', fig.keep='all', fig.height=5, message=FALSE}
options(warn = -1) 
library(microbiome)
library(phyloseq)
...
write.csv(asv_tax, paste(folder, "Upper control vs treatment.csv", sep=''))
