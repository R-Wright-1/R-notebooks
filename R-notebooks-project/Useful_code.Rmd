...
```{bash, eval=FALSE}
rsync --partial --progress W0.tar.bz2 user@server.address
```
...
Connect RStudio to server:
```{bash, eval=FALSE}
 ssh -N -L 8787:server.address:8787 user@server.address
```
Go to [this link](http://localhost:8787/). Login details are robyn and server password
...
[Server backup](https://github.com/LangilleLab/lab_information/wiki/Backups-and-Extra-Storage):
```{bash, eval=FALSE}
ln -s fungi_metagenome/processing/ robyn_backup/fungi_metagenome/
```
...
Based on the [standard workflow here](https://github.com/LangilleLab/microbiome_helper/wiki/Amplicon-SOP-v2-(qiime2-2020.2)).
...
Downloading from PGPC with check to see what kind of file it is first (and whether it already is downloaded):
```{python, eval=FALSE}
import os
import subprocess
...
humann_databases --download chocophlan full /home/vinko/databases/HUMANN/ --update-config yes
humann_databases --download uniref uniref90_diamond /home/vinko/databases/HUMANN/ --update-config yes
humann_databases --download uniref uniref50_diamond  /home/vinko/databases/HUMANN/ --update-config yes
humann_databases --download utility_mapping full /home/vinko/databases/HUMANN/ --update-config yes
```
...
Put the ticks on top of the axis:
```{python, eval=FALSE}
ax1.xaxis.tick_top()
```
...
Replace all numbers above a certain value:
```{python, eval=FALSE}
df[df > 9] = 11
```
...
Sort by value in certain column:
```{python, eval=FALSE}
db = db.sort_values(by=['col1'], ascending=True)
```
...
Make a sequence record:
```{python, eval=FALSE}
record = SeqRecord(Seq(sequence, IUPAC.protein),id='name', description='description')
```

Write a sequence file:
```{python, eval=FALSE}
SeqIO.write(list_of_sequences, "file_name.fasta", "fasta")
```

Read a sequence file (sequence by sequence, in this case adding to a dictionary):
```{python, eval=FALSE}
for record in SeqIO.parse('file_name.fasta', "fasta"):
    sequence_dict[record.id] = [record.description, record.seq]
```
...
Save a file:
```{python, eval=FALSE}
with open('file_name.dict', 'wb') as f:
    pickle.dump(dictionary, f)
```

Read a file:
```{python, eval=FALSE}
with open('file_name.dict', 'rb') as f:
